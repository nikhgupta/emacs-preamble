...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-capture-location (concat org-directory "/captures" ))

  ;; replace initial/scratch buffer with our primary `.org' file
  (setq initial-buffer-choice (concat org-directory "/start.org"))
  ...
  (setq org-log-done t                            ; record information when task is marked as DONE
        ...
        org-tags-column -145                      ; indent tags to the right
  ...
  (after 'org-mode (setq 'fill-column 145))       ; set column width to 100 for org-mode files
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-treat-S-cursor-todo-selection-as-state-change nil) ;; do not activate state changes on cycling

  (setq org-todo-keywords
        (quote ((sequence "TODO(t)" "STARTED(s)" "NEXT(n)" "|" "DONE(d@/!)")
                (sequence "REPORT(r!)" "BUG(b@/!)" "KNOWNCAUSE(k@/!)" "|" "FIXED(f@/!)")
                (sequence "WAITING(w@/!)" "HOLD(h@/!)" "SOMEDAY(S)" "PROJECT(P@)" "|" "CANCELLED(c@/!)" "PHONE" "MEETING"))))
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-journal-file (concat org-directory "/personal/diary.org"))
  (setq org-default-notes-file (concat org-capture-location "/refile.org"))
  (setq org-tasks-capture-file (concat org-capture-location "/tasks.org"))
  (setq org-links-capture-file (concat org-capture-location "/links.org"))

  (defun org-journal-entry-time() (format-time-string "%I:%M %p"))

  ;; customizations for `org-capture'
  (setq org-capture-templates
        '(("t" "Todo" entry (file+headline org-tasks-capture-file "Tasks")
           "* TODO %?\n%i\n")
          ("n" "Note" entry (file+datetree org-default-notes-file "Notes")
           "* %? :NOTE:\n%U\n%a\n%?" :clock-in t :clock-resume t)
          ("j" "Journal" entry (file+datetree org-journal-file "Journal")
           "* at %(org-journal-entry-time)\n  %U\n  %?" :clock-in t :clock-resume t)
          ("l" "Link" plain (file org-links-capture-file "Links")
           "- %?\n %x\n")
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "<f11>") 'preamble/ido-choose-from-recentf)
